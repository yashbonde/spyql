...
class Q:
    def __init__(
        self, query: str, input_options: dict = {}, output_options: dict = {}
    ) -> None:
        """
        ...
        if _from and os.path.exists(_from):
        ...
        if _to == None:
            ...
            if _to.upper() in Writer._valid_writers:
                raise SyntaxError(
                    f"Cannot export to a writer format in interactive mode: '{_to}'"
                )
        ...
        self.processor = Processor.make_processor(
            prs=self.parsed, strings=self.strings, input_options=input_options
        )
    ...
    def __call__(self, **kwargs):
        ...
        out = self.processor.go(
            output_file=f, output_options=self.output_options, user_query_vars=kwargs
        )
