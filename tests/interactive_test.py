...
def test_distinct_exploding():
    ...
    res = Q(f"SELECT DISTINCT data FROM data EXPLODE data->a TO {target}",)(
        data=[
            {
                "a": [1, 2, 2],
                "b": "three",
...
def test_ux():
    _q = Q(
        "SELECT data->name as first_name, data->age as user_age FROM data WHERE"
        " data->age > 30"
    )
    ...
    # using SpyQL
    out = Q(
        "SELECT sum_agg(data->salary) / len(data) as sum_salary FROM data WHERE"
        " data->age >= 30"
    )(data=raw_data)
...
def test_readme():
    ...
    query = Q(
        "SELECT data->invoice_num AS id, data->items->name AS name, data->items->price"
        " AS price FROM data EXPLODE data->items"
    )
    out = query(
        data=[
            {
    ...
    query = Q("SELECT 10 * cos(col1 * ((pi * 4) / 90)) FROM range(80)")
    out = query()

    query = Q("SELECT * FROM [10 * cos(i * ((pi * 4) / 90)) for i in range(80)]")
    out = query()
